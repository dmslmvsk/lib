...
function ShelvesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { label: string; libraryId: string; genreId: string }) =>
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: string
      data: { label: string; libraryId: string; genreId: string }
  ...
  const columns: TableColumn<Shelf>[] = [
    ...
    { header: "Books", render: (shelf) => shelf.books?.length || 0 },
    {
      header: "ID",
      render: (shelf) => (
        <span className="font-mono text-xs text-zinc-500">{shelf.id}</span>
      ),
    },
  ]
  ...
  if (isError)
    return <div className="p-4 text-red-500">Error loading shelves.</div>
      ...
      <AdminModal
        ...
        description={
          editingItem
            ...
            : "Create a new shelf and assign a library and genre."
        }
        fields={modalFields}
